...
\documentclass[finnish]{tktltiki2}
  ...
  \subsection{Keinotekoinen neuroni}
    ...
    Keinotekoiset neuronit ottavat vastaan yhden tai useampia syötteitä $x_i$ ... $x_i$, joista kullakin on jokin painoarvo $w_i$. Neuroneilla on yksi ulostuloarvo $y$, joka muodostetaan sen syötteistä kahdessa vaiheessa.

    %Ensimmäisessä vaiheessa syötteet litistetään yhdeksi arvoksi kertomalla ne painoarvollaan, summaamalla ne $\Sigma w_i x_i$, ja lisäämällä taipumusvakion (bias) $b$ tähän summaan.
    %Ensimmäisessä vaiheessa syötteet litistetään yhdeksi arvoksi summaamalla syötteet, joista kukin kerrotaan painoarvollaan, ja summaan lisätään lopuksi taipumusvakio (bias) $b$.
    Ensimmäisessä vaiheessa syötteet kerrotaan painotusarvollaan ja litistetään yhdeksi arvoksi summaamalla. Summaan lisätään lopuksi painotusarvo $b$.
  ...
  \end{figure}
    ...
    Aktivaatiofunktioina käytetään usein derivoituvia funktioita, sillä tämä helpottaa myöhemmin kappaleessa \ref{chap:backprop} esiteltävän takaisinvirtausalgoritmin käyttöä. Eräs suosittu aktivaatiofunktio on sigmoidinen funktio
